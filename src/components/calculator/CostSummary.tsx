          ...
          <div className="space-y-3 mb-6">
            {coloCost > 0 && (
              <div className="flex justify-between">
                <span className="text-white/70">{categoryLabels.colocation}</span>
                <span className="font-medium">{formatCAD(coloCost)}{dictionary.perMonth}</span>
              </div>
            )}
            {serverCost > 0 && (
              <div className="flex justify-between">
                <span className="text-white/70">{categoryLabels.server}</span>
                <span className="font-medium">{formatCAD(serverCost)}{dictionary.perMonth}</span>
              </div>
            )}
            {managedCost > 0 && (
              <div className="flex justify-between">
                <span className="text-white/70">{categoryLabels.managed}</span>
